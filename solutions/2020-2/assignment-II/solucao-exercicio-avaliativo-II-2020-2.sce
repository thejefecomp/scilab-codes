...
Este programa fornece uma possível solução para o enunciado do Exercício Avaliativo II da disciplina de Programação para Engenharia II, a utilizar como base a solução do Exercício Avaliativo I. Além das funcionalidades fornecidas pela calculadora S-I, implementada como solução do Exercício Avaliativo I, a calculadora S-II fornece a possibilidade de realizar multiplicação vetorial e matricial, e geração de gráficos de funções pré-definidas.
...
function imprimeCalculo(mensagem,nome,varargin)
    ...
    if numeroEntradas == 5 then
        ...
        if numeroResultados == 0 && varargin(3) then
            mprintf(varargin(1))
        end
   end
...
5 -> O utilizador desea gerar gráficos de funções pré-definidas;

6 -> O utilizador deseja realizar exibir o último cálculo realizado;

7 -> O utilizador deseja limpar o console do scilab;

8 -> O utilizador deseja finalizar o programa.
...
menuInicial = list('1 - Realizar cálculo com dois operandos','2 - Realizar cálculo com três operandos', '3 - Multiplicar vetores', '4 - Multiplicar matrizes', '5 - Gerar gráficos de funções pré-definidas', '6 - Exibir o último cálculo realizado', '7 - Limpar console', '8 - Finalizar programa')
...
while opcao <> 8
    ...
    select opcao
        ...
        case 5 then geraGraficosFuncoes(nome)
    ...
    if (opcao > 0 && opcao  < 5) || (opcao == 6 && ultimoCalculo) then
